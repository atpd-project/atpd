...
int config_load(const char *path, atp_config_t *cfg) {
    ...
    int validation_result = config_validate_values(cfg);

    pthread_mutex_unlock(&cfg->config_mutex);

    if (validation_result != 0) {
        LOG_WARN("Configuration file has errors, some settings may not work correctly");
    } else {
        LOG_INFO("Configuration loaded and validated from %s", path);
    }
...
int config_save(const char *path, atp_config_t *cfg) {
    ...
    fprintf(fp, "# Generated by atpd v" ATP_VERSION "\n\n");
    ...
    fprintf(fp, "UI_EMOJI_ENABLED=%d\n", cfg->ui_emoji_enabled);
...
void config_print_summary(atp_config_t *cfg) {
    ...
    LOG_INFO("  UI Emoji: %s", cfg->ui_emoji_enabled ? "enabled" : "disabled (ASCII)");
