...
static int init_netlink_event_socket(void) {
    if (g_nl_event_fd >= 0) return g_nl_event_fd;
...
static void fire_callbacks_and_free(iface_wait_ctx_t *head, int success) {
    while (head) {
        ...
        if (head->callback) head->callback(head->ifname, success, head->userdata);
        free(head);
        head = next;
    }
}
...
static void process_netlink_events(void) {
    ...
    if (completed_list) fire_callbacks_and_free(completed_list, 1);
}

static void netlink_event_cb(reactor_t *r, int fd, uint32_t events, void *userdata) {
    (void)r; (void)fd; (void)events; (void)userdata;
...
static void wait_timer_cb(reactor_t *r, reactor_timer_t *timer, void *userdata) {
    ...
    pthread_mutex_unlock(&g_list_mutex);
    if (extracted_ctx) fire_callbacks_and_free(extracted_ctx, success);
}

int netlink_wait_for_iface(reactor_t *r, const char *ifname, uint32_t timeout_ms, netlink_wait_cb callback, void *userdata) {
    if (!r || !ifname || !*ifname || !callback) return -1;
...
void netlink_wait_cleanup(void) {
    ...
    while (ctx) {
        ...
        if (ctx->timer && ctx->reactor) reactor_cancel_timer(ctx->reactor, ctx->timer);
        free(ctx);
        ctx = next;
    }
    
    if (g_nl_event_fd >= 0) {
        if (g_nl_event_registered && g_atpd.reactor) {
            reactor_remove_fd(g_atpd.reactor, g_nl_event_fd);
        }
        close(g_nl_event_fd);
        g_nl_event_fd = -1;
